...
void save_inodes(const char *master_file_table, struct inode *root) {
    if (!master_file_table || !root) {
        debug(__func__, "invalid arguments", "");
        return;
    }

    // First pass: map each inode pointer to its ID
    typedef struct {
        struct inode *node;
        uint32_t id;
    } NodeMapping;

    size_t map_capacity = 32;
    size_t map_size = 0;
    NodeMapping *node_map = malloc(map_capacity * sizeof(NodeMapping));
    
    if (!node_map) {
        debug(__func__, "failed to allocate memory for node map", "");
        return;
    }
    ...
    size_t stack_size = 0;
    StackItem *stack = malloc(stack_capacity * sizeof(StackItem));
    
    if (!stack) {
        debug(__func__, "failed to allocate memory for traversal stack", "");
        free(node_map);
        return;
    }
    ...
    if (stack_capacity == 0) {
        stack = realloc(stack, sizeof(StackItem));
        stack_capacity = 1;
    }
    stack[0].node = root;
    stack[0].visited = 0;
    ...
    // Iterative traversal to build the node map
    while (stack_size > 0) {
        // Get current node from top of stack
        StackItem *current = &stack[stack_size - 1];
        
        if (!current->visited) {
            // First visit to this node - add to map
            if (map_size >= map_capacity) {
                map_capacity *= 2;
                NodeMapping *new_map = realloc(node_map, map_capacity * sizeof(NodeMapping));
                if (!new_map) {
                    debug(__func__, "failed to resize node map", "");
                    free(stack);
                    free(node_map);
                    return;
                }
            ...
            node_map[map_size].node = current->node;
            node_map[map_size].id = current->node->id;
            map_size++;
            
            current->visited = 1;
            
            // If it's a directory, push all children onto the stack
            if (current->node->is_directory && current->node->num_entries > 0) {
                // Make sure we have enough stack space
                if (stack_size + current->node->num_entries > stack_capacity) {
                    stack_capacity = stack_size + current->node->num_entries;
                    StackItem *new_stack = realloc(stack, stack_capacity * sizeof(StackItem));
                    if (!new_stack) {
                        debug(__func__, "failed to resize stack", "");
                        free(stack);
                        free(node_map);
                        return;
                    }
                    stack = new_stack;
                    current = &stack[stack_size - 1]; // Update pointer after realloc
                }
                
                // Push children in reverse order so they're processed in the original order
                for (int i = current->node->num_entries - 1; i >= 0; i--) {
                    stack[stack_size].node = (struct inode *)current->node->entries[i];
                    stack[stack_size].visited = 0;
                    stack_size++;
                }
            } else {
                // Pop the node since we're done with it
                stack_size--;
            }
        } else {
            // Second visit - finished with children, pop the node
            stack_size--;
        }
    }
    ...
    if (!file) {
        debug(__func__, "failed to open file for writing:", master_file_table);
        free(node_map);
        return;
    }

    // Second pass: write each node to the file
    for (size_t i = 0; i < map_size; i++) {
        struct inode *node = node_map[i].node;
        
        // Write ID
        fwrite(&node->id, sizeof(uint32_t), 1, file);
        
        // Write name length and name
        uint32_t name_length = strlen(node->name) + 1; // Include null terminator
        fwrite(&name_length, sizeof(uint32_t), 1, file);
        fwrite(node->name, sizeof(char), name_length, file);
        
        // Write directory and readonly flags
        fwrite(&node->is_directory, sizeof(char), 1, file);
        fwrite(&node->is_readonly, sizeof(char), 1, file);
        
        // Write filesize for files
        if (!node->is_directory) {
        ...
        // Write entries array
        if (node->is_directory) {
            // For directories, we need to convert pointers back to IDs
            uintptr_t *id_entries = malloc(node->num_entries * sizeof(uintptr_t));
            if (!id_entries) {
                debug(__func__, "failed to allocate memory for ID entries", "");
                fclose(file);
                free(node_map);
                return;
            }
            ...
            for (uint32_t j = 0; j < node->num_entries; j++) {
                struct inode *child = (struct inode *)node->entries[j];
                
                // Find the child's ID in our map
                for (size_t k = 0; k < map_size; k++) {
                    if (node_map[k].node == child) {
                        id_entries[j] = node_map[k].id;
                        break;
                    }
                }
            }
            
            fwrite(id_entries, sizeof(uintptr_t), node->num_entries, file);
            free(id_entries);
        } else {
    ...
    fclose(file);
    free(node_map);
    debug(__func__, "filesystem saved successfully to", master_file_table);
}
